...
def main(**args):
    ...
    model_params = dict(model_path=args.get('model_folder'),
                        ...
                        create_transl=True,
                        # flat_hand_mean=args.get('flat_hand_mean'),
    ...
    # Get bio_contact for this frame (passed as argument)
    bio_contact = int(args.pop('bio_contact'))
        
    # Get MultiHMR initialization folder
    init_body_folder = args.pop('init_body_folder')

    verbose = args.pop('verbose', False)
    
    # Remove init_pose_path from args if present (we'll construct it ourselves)
    args.pop('init_pose_path', None)
    
    img_folder = args.pop('img_folder', 'images')
    dataset_obj = create_dataset(img_folder=img_folder, **args)
    
    # Should have exactly 1 frame
    assert len(dataset_obj) == 1, f"Expected 1 frame, got {len(dataset_obj)}"
    
    for data in dataset_obj:
        img = data['img']
        fn = data['fn']
        keypoints = data['keypoints'].copy()  # Make a copy to avoid modifying the original
        
        # Continue with the rest of the code using the potentially resized image
        if args['interactive']:
        ...
        # bio_contact is passed as a parameter (0 or 1)
        
        # Construct path to initialization file for this frame
        # Try common patterns: frame_{fn}.pkl or {fn}.pkl
        init_pose_path = None
        for pattern in [f"frame_{fn}.pkl", f"{fn}.pkl", f"frame_{int(fn):06d}.pkl"]:
            candidate_path = osp.join(init_body_folder, pattern)
            if osp.exists(candidate_path):
                init_pose_path = candidate_path
                break
        
        if init_pose_path is None:
            raise FileNotFoundError(f"No initialization file found for frame {fn} in {init_body_folder}")
